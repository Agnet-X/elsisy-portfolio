      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-20">
          <div className="md:col-span-2">
            ...
            <p className="text-white/60 max-w-sm mb-8 font-light leading-relaxed">
              {lang === 'ar'
                ? 'صياغة الروايات البصرية وتوثيق اللحظات التي تتجاوز الزمن. تصوير ومونتاج احترافي في الإمارات.'
          ...
          <div>
            <h4 className="text-white font-serif text-xl mb-6">
              ...
              <li>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <a href={CONTENT.identity.social.instagram} target="_blank" rel="noreferrer" className="text-white/50 hover:text-primary transition-colors">
                      <SiInstagram size={14} />
                    </a>
                    <a href={CONTENT.identity.social.tiktok} target="_blank" rel="noreferrer" className="text-white/50 hover:text-primary transition-colors">
                      <SiTiktok size={13} />
                    </a>
                  </div>
                </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-white/60 font-light">
              ...
              <li>
                <a href={`tel:${CONTENT.identity.phone.replace(/[^0-9+]/g, '')}`} className="hover:text-primary transition-colors">
                  {CONTENT.identity.phone}
                </a>
              </li>
              <li>
                <a href={`mailto:${CONTENT.identity.email}`} className="hover:text-primary transition-colors">
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-white/40 text-sm">
            &copy; {new Date().getFullYear()} {CONTENT.identity.name}. {tf.rights}
          </p>
          <p className="text-white/40 text-sm font-serif italic">{tf.tagline}</p>
        </div>
      </div>
    </footer>
  );
